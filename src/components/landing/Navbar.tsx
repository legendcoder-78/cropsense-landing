...
const Navbar = ({ alwaysOpaque = false }: { alwaysOpaque?: boolean }) => {
  ...
  return (
    <>
        ...
        <div className="container mx-auto flex items-center justify-between py-3 px-6">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            {user ? (
              <Link
                to="/dashboard"
                className="flex items-center gap-2 rounded-full bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground transition-all duration-200 hover:brightness-110 hover:scale-105"
              >
          ...
          <div className="md:hidden bg-background/95 backdrop-blur-md border-t border-border px-6 pb-6 pt-2 animate-fade-up">
            ...
            {user ? (
              <Link
                to="/dashboard"
                onClick={() => setMobileOpen(false)}
                className="block py-3 font-body text-primary font-semibold hover:text-primary/80 transition-colors"
              >
                Dashboard
              </Link>
            ) : (
      ...
      <AuthModal open={isModalOpen} onOpenChange={closeModal} defaultTab={modalMode} />
    </>
  );
};
